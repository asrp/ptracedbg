...
def memory_maps():
    ...
    mmaped_counter = 0
    for line in open("/proc/%s/maps" % process.pid).readlines():
        if len(line.split()) == 5:
...
def load_lib_vars(filename):
    line1 = [l for l in open("/proc/%s/maps" % process.pid).readlines()
...
# Remember to update the post
# Problem was the manually set rip not unset
